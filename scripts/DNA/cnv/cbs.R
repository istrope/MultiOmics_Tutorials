...
option_list = list(
	make_option(c('-i','--indir'),type='character',default = '.',help = "input directory"),
	make_option(c('-o','--outdir'),type='character',default='.',help='output directory'),
	make_option(c("-b","--bad.bins"),type='character',default="hg38/hg38.50k.k50.bad.bins.txt",help="bad bins file"),
	make_option(c("-v","--varbin.gc"),type="character",default = "hg38/hg38.varbin.gc.content.50k.bwa.k50.txt"),
	make_option(c("-d","--varbin.data"),type="character",default = NULL,help="varbin data for sample of interest"),
	make_option(c("-s","--sample.name"),type="character",default=NULL,help="Sample Name")
	)
...
cbs.segment01 <- function(indir, outdir, bad.bins, varbin.gc, varbin.data, sample.name, alt.sample.name, alpha, nperm, undo.SD, min.width) {
	...
	write.table(thisRatio, sep="\t", file=paste(outdir, "/", sample.name, ".hg38.50k.k50.varbin.data.txt", sep=""), quote=F, row.names=F) 
	write.table(thisShort, sep="\t", file=paste(outdir, "/", sample.name, ".hg38.50k.k50.varbin.short.txt", sep=""), quote=F, row.names=F) 
	...
	write.table(thisRatioNobad, sep="\t", file=paste(outdir, "/", sample.name, ".hg38.50k.k50.nobad.varbin.data.txt", sep=""), quote=F, row.names=F) 
	write.table(thisShort, sep="\t", file=paste(outdir, "/", sample.name, ".hg38.50k.k50.nobad.varbin.short.txt", sep=""), quote=F, row.names=F) 

}
cbs.segment01(indir=opt$indir, 
	outdir=opt$outdir, 
	bad.bins=opt$bad.bins,
	varbin.gc=opt$varbin.gc,
	varbin.data=opt$varbin.data,
